...
def get_weight_path():
    return os.path.join('keras_frcnn', 'weights','md_inception_resnet_v2.h5')
...
def get_img_output_length(width, height):
    def get_output_length(input_length):
        ...
        strides = [2, 1, 2, 1, 2, 2]
        dilation = [1, 1, 1, 1, 1, 2]

        assert len(filter_sizes) == len(strides)

        for i in range(len(filter_sizes)):
            input_length = (input_length - filter_sizes[i])*dilation[i] // strides[i] + 1
        return input_length
...
def inception_resnet_block(x, scale, block_type, block_idx, use_atrous=False, activation='relu'):
    ...
    if block_type == 'block35':
        ...
        branch_0 = conv2d_bn(x, 192, 1, dilation_rate=dr, name=block_name + '_conv1')
        branch_1 = conv2d_bn(x, 128, 1, dilation_rate=dr, name=block_name + '_conv2')
        branch_1 = conv2d_bn(branch_1, 160, [1, 7], dilation_rate=dr, name=block_name + '_conv3')
        branch_1 = conv2d_bn(branch_1, 192, [7, 1], dilation_rate=dr, name=block_name + '_conv4')
...
def inception_resnet_block_td(x, scale, block_type, block_idx, use_atrous=False, activation='relu'):
    """Adds a Inception-ResNet block.
    This function builds 3 types of Inception-ResNet blocks mentioned
    in the paper, controlled by the `block_type` argument (which is the
    block name used in the official TF-slim implementation):
    ...
    if block_type == 'block35':
        ...
        branch_0 = conv2d_bn_td(x, 192, 1, dilation_rate=2 if use_atrous else 1, name=block_name + '_conv1')
        branch_1 = conv2d_bn_td(x, 128, 1, dilation_rate=2 if use_atrous else 1, name=block_name + '_conv2')
        branch_1 = conv2d_bn_td(branch_1, 160, [1, 7], dilation_rate=2 if use_atrous else 1, name=block_name + '_conv3')
        branch_1 = conv2d_bn_td(branch_1, 192, [7, 1], dilation_rate=2 if use_atrous else 1, name=block_name + '_conv4')
        branches = [branch_0, branch_1]
    elif block_type == 'block8':
...
#    if K.image_dim_ordering() == 'tf':
    ...
    branch_0 = conv2d_bn(x, 384, 3, strides=1 if use_atrous else 2, padding='valid', name='Reduction_A_block' + '_conv1')
    branch_1 = conv2d_bn(x, 256, 1, name='Reduction_A_block' + '_conv2')
    branch_1 = conv2d_bn(branch_1, 256, 3, name='Reduction_A_block' + '_conv3')
    branch_1 = conv2d_bn(branch_1, 384, 3, strides=1 if use_atrous else 2, padding='valid', name='Reduction_A_block' + '_conv4')
    branch_pool = MaxPooling2D(3, strides=1 if use_atrous else 2, padding='valid')(x)
    ...
    for block_idx in range(1, 21):
        x = inception_resnet_block(x,
                                   scale=0.1,
                                   block_type='block17',
                                   block_idx=block_idx, use_atrous=use_atrous)
